...
export default function TransactionForm({
    ...
    const [amount, setAmount] = useState<string | undefined>(
        existingTransaction.amount ? `${existingTransaction.amount}` : void 0,
    );
    const [category, setCategory] = useState<SpendingCategoryType | ''>(
        existingTransaction.category || SpendingCategoryType.OTHER,
    );
    ...
    const [selectedDate, setSelectedDate] = useState<Dayjs>(
        existingTransaction.date ? dayjs(existingTransaction.date) : dayjs(),
    ); // defaults to today
    ...
    return (
        <SlideUpPanel
            title={getContent(editMode ? 'EDIT_EXPENSE' : 'NEW_EXPENSE')}
                    ...
                    <>
                        {!editMode && !tripSelected && activeTrip && (
                            <div className={styles.alertContainer}>
                                <Alert
                                    alertText={getContent('ACTIVE_TRIP', [
                                        activeTrip.tripName,
                        ...
                        <form className={styles.transactionForm}>
                            ...
                            <label htmlFor="date-input">{getContent('DATE_LABEL')}</label>
                            <DatePicker
                                disableFuture
