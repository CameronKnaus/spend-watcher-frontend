...
export default function Spending() {
    ...
    const {
        ...
        queryKey: [spendingSummaryQueryKey],
        queryFn: () => {
    ...
    const transactionsRequiringUpdate = spendingData?.recurringSpending?.recurringTransactions?.filter(
        (transaction) => transaction.requiresUpdate,
    );
    ...
    return (
        <div className={styles.spendingContainer}>
            <h2 className={`header-text ${styles.title}`}>{getContent('MY_SPENDING')}</h2>
            ...
            />
            <ActionTile
                isInactive={spendingData?.recurringSpending.noTransactions}
            ...
            />
            <ThickActionButton
                buttonColor="var(--theme-red-dark)"
