        ...
        <div className={styles.contentContainer}>
            <div className={styles.totalsGroup}>
                <div className={styles.totalSpentContainer}>
                    <div className={styles.totalLabel}>{getContent('TOTAL_SPENT')}</div>
                    <div className={styles.totalValue}>{`-${formatCurrency(spendingBreakdown.finalTotalSpent)}`}</div>
                </div>
                <div className={styles.totalSpentContainer}>
                    <div className={styles.totalLabel}>{getContent('TOTAL_DISCRETIONARY')}</div>
                    <div className={styles.totalValue}>
                        {`-${formatCurrency(spendingBreakdown.discretionaryTotal)}`}
                    </div>
                </div>
                <div className={styles.totalSpentContainer}>
                    <div className={styles.totalLabel}>{getContent('TOTAL_RECURRING')}</div>
                    <div className={styles.totalValue}>
            ...
            {spendingBreakdown.finalTotalTransactions > 0 && (
                <>
                    ...
                    <div className={styles.topCategories}>
                        <TopSpendingCategories
                            label={getContent('CATEGORY_TOTAL_AMOUNT_TITLE')}
