...
export default function SpendCategoryFilter({
    filterCategory,
    setFilterCategory,
    textInputStyles,
}: SpendCategoryFilterPropTypes) {
    const getContent = useContent('SPENDING_BREAKDOWN');

    return (
        <>
            <div className={styles.filterLabel}>
                <label htmlFor="category-input">{getContent('FILTER_BY_CATEGORY')}</label>
            </div>
