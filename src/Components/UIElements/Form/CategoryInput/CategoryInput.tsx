...
export function AccountCategoryInput({
    id = 'category-input',
    textInputStyles,
    value,
    onChange,
}: AccountCategoryInputPropTypes) {
                ...
                <div className={`${styles.options} low-shadow`}>
                    {
                        ...
                        filter(Object.keys(AccountCategoryType)).map((category) => (
                            ...
                            >
                                <CategoryIcon categoryCode={category} containerSize="2rem" iconSize="1.2rem" />
...
// This is a controlled-only component.  State must be managed by the parent
export function SpendingCategoryInputs({
    id = 'category-input',
    defaultNoSelectionToOther = false,
    textInputStyles,
    value,
    onChange,
}: SpendingCategoryInputPropTypes) {
    const [open, setOpen] = useState(false);
    const [filterText, setFilterText] = useState('');
    const ref = useRef(null);
    const getContent = useContent('GENERAL');
    ...
    return (
        <div className={styles.dropdown}>
            ...
            {open && (
                <div className={`${styles.options} low-shadow`}>
                    {
                        // @ts-expect-error TODO: Revisit Typescript
